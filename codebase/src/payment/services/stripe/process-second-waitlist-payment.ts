import Stripe from 'stripe';
import { PROMOTIONAL_CODE, STRIPE_API_KEY, WEBSITE_URL } from 'src/config';
import { Injectable } from '@nestjs/common';
import { DataSource } from 'typeorm';
import { KitType, PaymentGateway, PaymentType } from 'src/enum';
import { Transaction } from 'src/payment/entity/transaction.entity';
import { ShotgunWaitlist } from 'src/order/entity/shotgun-waitlist.entity';
import { getPriceId } from 'src/common/utils/manage-pricing';
import { InjectDataSource } from '@nestjs/typeorm';

@Injectable()
export class StripeProcessSecondWaitlistPayment {
  private stripe: Stripe;
  private readonly apiKey = STRIPE_API_KEY;

  constructor(@InjectDataSource() private readonly dataSource: DataSource) {
    this.stripe = new Stripe(this.apiKey, { apiVersion: '2024-06-20' });
  }
  async execute(
    referenceId: string,
    initialReferenceId: string,
    quantity: number,
    priceId?: string,
  ): Promise<Stripe.Checkout.Session> {
    try {
      const session = await this.dataSource.transaction(async (manager) => {
        const line_items: Stripe.Checkout.SessionCreateParams.LineItem[] = [];

        const waitlist = await manager.findOne(ShotgunWaitlist, {
          where: { referenceId: initialReferenceId },
        });

        const currency = waitlist.currency;
        const selectedPrice = priceId ?? getPriceId(KitType.DeepGut, currency);
        quantity = quantity ?? 1;

        if (selectedPrice) {
          line_items.push({ price: selectedPrice, quantity });
        } else {
          throw new Error('No valid price ID found');
        }

        const customerId = waitlist.stripeCustomerId;

        // Prepare client information
        const clientName =
          `${waitlist.firstName || ''} ${waitlist.lastName || ''}`.trim() ||
          'N/A';
        const clientEmail = waitlist.email || 'N/A';

        waitlist.finalReferenceId = referenceId;

        const [session, transaction] = await Promise.all([
          this.stripe.checkout.sessions.create({
            success_url: `${WEBSITE_URL}/payment-success?status=success&session_id={CHECKOUT_SESSION_ID}`,
            line_items,
            mode: 'payment',
            customer: customerId,
            client_reference_id: referenceId,
            allow_promotion_codes:
              PROMOTIONAL_CODE === 'enabled' ? true : false,
            payment_intent_data: {
              setup_future_usage: 'off_session',
            },
            metadata: {
              clientName: clientName,
              clientEmail: clientEmail,
              referenceId: referenceId,
              initialReferenceId: initialReferenceId,
              country: waitlist.country,
              currency: currency,
            },
          }),
          manager.save(Transaction, {
            referenceId,
            type: PaymentType.Pay,
            gateway: PaymentGateway.Stripe,
          }),
          manager.save(waitlist),
        ]);

        return session;
      });

      return session;
    } catch (error) {
      console.error('Payment processing error:', error);
      throw error;
    }
  }
}
