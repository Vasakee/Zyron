...
@Injectable()
export class GetWaitlistOrderService {
  private readonly logger = new Logger(GetWaitlistOrderService.name);
  ...
  async execute(pageOptionsDto: PageOptionsDto, query: OrdersQueryDto) {
    try {
      ...
      const dbQuery: SelectQueryBuilder<Order> = this.orderRepo
        ...
        .where('orders.source = :source', { source: Source.Waitlist });
      ...
      const [response, totalCount, counts] = await Promise.all([
        dbQuery
          ...
          .getManyAndCount(),
        this.orderRepo.count({ where: { source: Source.Waitlist } }),
        this.orderRepo
          .createQueryBuilder('orders')
