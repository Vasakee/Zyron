...
@Injectable()
export class ResendNewPlatformPasswordResetService {
  private readonly logger = new Logger(
    ResendNewPlatformPasswordResetService.name,
  );
  constructor(
  ...
  async execute(data: ForgotPasswordDTO) {
    ...
    const link = `${FRONTEND_URL}/access-password-reset?token=${token}&email=${email}`;
    const mailData = {
      name: user.firstName,
      email: user.email,
      link,
