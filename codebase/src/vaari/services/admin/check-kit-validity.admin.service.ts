...
@Injectable()
export class CheckKitValidityAdminService {
  private readonly logger = new Logger(CheckKitValidityAdminService.name);
  ...
  async execute(kitId: string, userId: string) {
    try {
      ...
      throw new NotFoundException(`No kit found with ID "${kitId}" `);
    } catch (error) {
